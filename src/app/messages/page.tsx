...
export default function MessagesPage() {
	...
	const handleGenerateDraft = async (intent: string) => {
		...
		const materialContext = posts.find(
			(m) => m.owner === selectedConversation.participant.id
		);
		if (!materialContext) {
			toast({
				title: "Cannot Generate Draft",
				description:
					"Could not find a sample item from this seller to discuss.",
				variant: "destructive",
			});
			return;
		}
		...
		try {
			...
			if (result.messageDraft) setNewMessage(result.messageDraft);
				...
				toast({
					...
					description:
						"The AI could not generate a message draft. Please try again.",
					variant: "destructive",
				});
		} catch {
			toast({
					...
					<ScrollArea className="flex-grow">
						<div className="space-y-1 p-2">
							{conversations.map((convo) => {
								const otherUser = convo.users.find(
							...
							<ScrollArea className="flex-grow min-h-0 bg-muted/50 p-4">
								<Alert className="mb-4 bg-primary/10 border-primary/20">
									...
									<AlertDescription className="text-primary/80">
										Keep your conversations and transactions on PlanetBuild to
										stay protected. Never share personal contact information.
									</AlertDescription>
								</Alert>

								<div className="space-y-4">
									{chatHistory.map((msg) => {
										const sender = selectedConversation.users.find(
											(u: any) => u.userId === msg.sentUserId
										);
										const isCurrentUser = msg.sentUserId === currentUser._id;
										const repliedMsg = msg.replyTo
											? chatHistory.find((m) => m._id === msg.replyTo)
											: null;
										const reactionGroups =
											msg.reactions?.reduce(
												(acc: Record<string, string[]>, r: Reaction) => {
											...
											<div
												...
												<div
														...
														<div className="text-xs text-gray-900 bg-gray-200 border-l-2 border-primary p-2 mb-1 rounded opacity-90">
															<span className="font-semibold">
																{selectedConversation.users.find(
																	(u: any) => u.userId === repliedMsg.sentUserId
																)?.name || "User"}
																:
															</span>{" "}
															{repliedMsg.text}
													...
													<div
														...
														className={cn(
															"max-w-xs md:max-w-md p-1 rounded-lg relative",
															isCurrentUser
																? "bg-primary text-primary-foreground rounded-br-none"
																: "bg-card rounded-bl-none"
														)}
														onMouseEnter={() => {
															// Cancel any pending close if we re-enter
															if (closeTimerRef.current) {
																clearTimeout(closeTimerRef.current);
																closeTimerRef.current = null;
															}
														...
														onMouseLeave={() => {
															// Delay closing to allow cursor to enter the popup
															if (closeTimerRef.current) {
																clearTimeout(closeTimerRef.current);
															}
														...
														{Object.entries(reactionGroups).map(
															([emoji, users]) => (
																<span
																	key={emoji}
																	className={cn(
																		"px-1 py-0.5 rounded text-xs cursor-pointer",
														...
														{Object.keys(reactionGroups).length > 0 && (
															<span
																className="px-1 py-1 rounded text-xs cursor-pointer hover:"
														...
														{showEmojiPicker === msg._id &&
															createPortal(
																<div
																	...
																	onMouseLeave={() => {
																		// Close shortly after leaving the popup
																		if (closeTimerRef.current) {
																			clearTimeout(closeTimerRef.current);
																		}
																		closeTimerRef.current = window.setTimeout(
								...
								<form
									...
									<DropdownMenu>
										...
										<DropdownMenuContent>
											...
											<DropdownMenuItem
												onSelect={() =>
													handleGenerateDraft("Ask about availability")
												}>
												Ask about availability
											</DropdownMenuItem>
											<DropdownMenuItem
												onSelect={() =>
													handleGenerateDraft("Ask for more details")
												}>
												Ask for more details
											</DropdownMenuItem>
											<DropdownMenuItem
												onSelect={() =>
													handleGenerateDraft("Arrange a viewing")
												}>
												Arrange a viewing
											</DropdownMenuItem>
											<DropdownMenuItem
												onSelect={() => handleGenerateDraft("Arrange pickup")}>
												Arrange pickup
											</DropdownMenuItem>
										</DropdownMenuContent>
									</DropdownMenu>
