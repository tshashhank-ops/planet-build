...
export default function AppSidebar() {
	...
	if (user) {
		...
		if (user.role === "seller" && user.organisation) {
			...
			menuItems.push({
				href: `/profile/${user._id}/active-listings`,
				label: "Active Listings",
				icon: PlusCircle,
			});
			menuItems.push({
				href: `/profile/${user._id}/reviews`,
				label: "Reviews",
				icon: Star,
			});
		}
	...
	return (
		<Sidebar variant="inset" collapsible="icon">
			<SidebarHeader>
				<Link
					href="/"
					className="flex items-center gap-2 text-lg font-bold text-sidebar-primary-foreground font-headline group-data-[state=collapsed]/sidebar-wrapper:justify-center">
