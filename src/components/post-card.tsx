...
export default function PostCard({ post }: PostCardProps) {
	// The seller data should now be populated directly from the API
	const seller = typeof post.seller === "object" ? post.seller : null;
							...
							<CardFooter className="p-4 pt-0 flex justify-between items-center">
								<div>
									...
									<p className="text-xs text-muted-foreground">
										{post.category === "Bricks" || post.category === "Flooring"
											? "per unit"
											: ""}
									</p>
								</div>
								...
								<div>
									...
									<div className="flex flex-wrap gap-1">
										{(seller.badges || []).map((badge: string) => (
											<EcoBadge key={badge} badgeName={badge} />
										))}
									</div>
								</div>
							</>
